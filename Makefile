...
export internalPKG := github.com/powersj/parquet/internal
...
.PHONY: build
build:
	CGO_ENABLED=0 go build -o parquet -ldflags "$(LDFLAGS)"

.PHONY: clean
clean:
	rm -rf parquet coverage.out dist/

.PHONY: help
help:
	@echo 'Available Targets:'
	@echo '  all      remove artifacts, lint and test code, then build the binary'
	@echo '  build    build the parquet binary'
